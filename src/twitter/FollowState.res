...
@react.component
let make = () => {
  let (state, event) = React.useReducer(reducer, FollowingHover)
  ...
  <>
    // <div>
    //   <button className={styles(~variant=getStyle(state))}> {React.string(text())} </button>
    // </div>
    // <div>
    //   <button
    //     onClick={_ => {
    ...
    //       event(MouseOut)
    //     }}
    //     className={styles(~variant=getStyle(state))}>
    //     {React.string(text())}
    //   </button>
    // </div>
    <div>
